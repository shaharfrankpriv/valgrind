...
static Bool clo_mem_log = False;    /* do memory logging? */
static const HChar* clo_cachegrind_out_file = "cachegrind.out.%p";
static const HChar* clo_cachegrind_mem_file = "cachegrind.mem.%p";
/*------------------------------------------------------------*/
...
static void get_debug_info(Addr instr_addr, const HChar** dir, const HChar** file, const HChar** fn, UInt* line)
{
    DiEpoch ep = VG_(current_DiEpoch)();
    Bool found_file_line = VG_(get_filename_linenum)(ep, instr_addr, file, dir, line);
...
static VG_REGPARM(1) void log_1Ir(InstrInfo* n)
{
    n->parent->Ir.a++;
}
...
static VG_REGPARM(1) void log_1IrGen_0D_cache_access(InstrInfo* n)
{
    ...
    cachesim_I1_doref_Gen(n->instr_addr, n->instr_len, &n->parent->Ir);
}
...
static VG_REGPARM(1) void log_1IrNoX_0D_cache_access(InstrInfo* n)
{
    ...
    //             n, n->instr_addr, n->instr_len);
    cachesim_I1_doref_NoX(n->instr_addr, n->instr_len, &n->parent->Ir);
}

static VG_REGPARM(2) void log_2IrNoX_0D_cache_access(InstrInfo* n, InstrInfo* n2)
{
    ...
    cachesim_I1_doref_NoX(n->instr_addr, n->instr_len, &n->parent->Ir);
    cachesim_I1_doref_NoX(n2->instr_addr, n2->instr_len, &n2->parent->Ir);
}

static VG_REGPARM(3) void log_3IrNoX_0D_cache_access(InstrInfo* n, InstrInfo* n2, InstrInfo* n3)
{
    ...
    cachesim_I1_doref_NoX(n->instr_addr, n->instr_len, &n->parent->Ir);
    cachesim_I1_doref_NoX(n2->instr_addr, n2->instr_len, &n2->parent->Ir);
    cachesim_I1_doref_NoX(n3->instr_addr, n3->instr_len, &n3->parent->Ir);
}

static VG_REGPARM(3) void log_1IrNoX_1Dr_cache_access(InstrInfo* n, Addr data_addr, Word data_size)
{
    ...
    cachesim_D1_doref(data_addr, data_size, &n->parent->Dr, ACCESS_READ);
}

static VG_REGPARM(3) void log_1IrNoX_1Dw_cache_access(InstrInfo* n, Addr data_addr, Word data_size)
{
    ...
    cachesim_D1_doref(data_addr, data_size, &n->parent->Dw, ACCESS_WRITE);
}
...
static VG_REGPARM(3) void log_0Ir_1Dr_cache_access(InstrInfo* n, Addr data_addr, Word data_size)
{
...
static VG_REGPARM(3) void log_0Ir_1Dw_cache_access(InstrInfo* n, Addr data_addr, Word data_size)
{
...
static SB_info* get_SB_info(IRSB* sbIn, Addr origAddr)
{
    ...
    sbInfo = VG_(OSetGen_AllocNode)(instrInfoTable, sizeof(SB_info) + n_instrs * sizeof(InstrInfo));
...
static InstrInfo* setup_InstrInfo(CgState* cgs, Addr instr_addr, UInt instr_len)
{
    InstrInfo* i_node;
    ...
    while (i < cgs->events_used) {
        ...
        case Ev_IrNoX:
            ...
            if (ev2 && (ev2->tag == Ev_Dr || ev2->tag == Ev_Dm)) {
                ...
                argv = mkIRExprVec_3(i_node_expr, get_Event_dea(ev2), mkIRExpr_HWord(get_Event_dszB(ev2)));
                regparms = 3;
                i += 2;
            }
            ...
            /* Merge an IrNoX with two following IrNoX's. */
                ...
                argv = mkIRExprVec_3(i_node_expr, mkIRExpr_HWord((HWord)ev2->inode), mkIRExpr_HWord((HWord)ev3->inode));
                regparms = 3;
                i += 3;
            }
        ...
        case Ev_Dm:
            ...
            argv = mkIRExprVec_3(i_node_expr, get_Event_dea(ev), mkIRExpr_HWord(get_Event_dszB(ev)));
            regparms = 3;
            i++;
            break;
        case Ev_Dw:
        ...
        tl_assert(argv);
        di = unsafeIRDirty_0_N(regparms, helperName, VG_(fnptr_to_fnentry)(helperAddr), argv);
...
static void addEvent_Dr(CgState* cgs, InstrInfo* inode, Int datasize, IRAtom* ea)
{
    Event* evt;
...
static void addEvent_Dw(CgState* cgs, InstrInfo* inode, Int datasize, IRAtom* ea)
{
    Event* evt;
    ...
    if (cgs->events_used > 0) {
        ...
        if (lastEvt->tag == Ev_Dr && lastEvt->Ev.Dr.szB == datasize && lastEvt->inode == inode &&
...
static void addEvent_D_guarded(CgState* cgs, InstrInfo* inode, Int datasize, IRAtom* ea, IRAtom* guard, Bool isWrite)
{
    tl_assert(isIRAtom(ea));
    tl_assert(guard);
    ...
    helperName = isWrite ? "log_0Ir_1Dw_cache_access" : "log_0Ir_1Dr_cache_access";
...
static void addEvent_Bc(CgState* cgs, InstrInfo* inode, IRAtom* guard)
{
    Event* evt;
    tl_assert(isIRAtom(guard));
    tl_assert(typeOfIRExpr(cgs->sbOut->tyenv, guard) == (sizeof(RegWord) == 4 ? Ity_I32 : Ity_I64));
    if (!clo_branch_sim)
        return;
    if (cgs->events_used == N_EVENTS)
        flushEvents(cgs);
...
static void addEvent_Bi(CgState* cgs, InstrInfo* inode, IRAtom* whereTo)
{
    ...
    tl_assert(typeOfIRExpr(cgs->sbOut->tyenv, whereTo) == (sizeof(RegWord) == 4 ? Ity_I32 : Ity_I64));
    if (!clo_branch_sim)
        return;
    if (cgs->events_used == N_EVENTS)
        flushEvents(cgs);
...
static IRSB* cg_instrument(VgCallbackClosure* closure, IRSB* sbIn, const VexGuestLayout* layout,
                           const VexGuestExtents* vge, const VexArchInfo* archinfo_host, IRType gWordTy, IRType hWordTy)
{
    Int i;
    UInt isize;
    IRStmt* st;
    ...
    for (/*use current i*/; i < sbIn->stmts_used; i++) {
        ...
        case Ist_IMark:
            ...
            // Sanity-check size.
            tl_assert((VG_MIN_INSTR_SZB <= isize && isize <= VG_MAX_INSTR_SZB) || VG_CLREQ_SZB == isize);
        ...
        case Ist_Store: {
            ...
            addEvent_Dw(&cgs, curr_inode, sizeofIRType(typeOfIRExpr(tyenv, data)), aexpr);
            break;
        }
        ...
        case Ist_StoreG: {
            ...
            addEvent_D_guarded(&cgs, curr_inode, sizeofIRType(type), addr, sg->guard, True /*isWrite*/);
            break;
        }
        ...
        case Ist_LoadG: {
            ...
            tl_assert(type != Ity_INVALID);
            addEvent_D_guarded(&cgs, curr_inode, sizeofIRType(type), addr, lg->guard, False /*!isWrite*/);
            break;
        }
        ...
        case Ist_Exit: {
            ...
            if ((st->Ist.Exit.jk == Ijk_Boring) || (st->Ist.Exit.jk == Ijk_Call) || (st->Ist.Exit.jk == Ijk_Ret)) {
                ...
                IRExpr* one = tyW == Ity_I32 ? IRExpr_Const(IRConst_U32(1)) : IRExpr_Const(IRConst_U64(1));
                ...
                addStmtToIRSB(cgs.sbOut, IRStmt_WrTmp(guardW, IRExpr_Unop(widen, IRExpr_RdTmp(guard1))));
                /* If the exit is inverted, invert the sense of the guard. */
                addStmtToIRSB(cgs.sbOut, IRStmt_WrTmp(guard, inverted ? IRExpr_Binop(opXOR, IRExpr_RdTmp(guardW), one)
                                                                      : IRExpr_RdTmp(guardW)));
                /* And post the event. */
...
static void fprint_CC_table_and_calc_totals(void)
{
    ...
    // happened in 3.3.0.
    HChar* cachegrind_out_file = VG_(expand_file_name)("--cachegrind-out-file", clo_cachegrind_out_file);

    fp = VG_(fopen)(cachegrind_out_file, VKI_O_CREAT | VKI_O_TRUNC | VKI_O_WRONLY, VKI_S_IRUSR | VKI_S_IWUSR);
    if (fp == NULL) {
    ...
    while ((lineCC = VG_(OSetGen_Next)(CC_table))) {
        ...
        if (clo_cache_sim && clo_branch_sim) {
            VG_(fprintf)(fp,
                         ...
                         " %llu %llu %llu %llu %llu"
                         " %llu %llu %llu %llu\n",
                         lineCC->loc.line, lineCC->Ir.a, lineCC->Ir.m1, lineCC->Ir.mL, lineCC->Ir.l1_words,
                         lineCC->Ir.llc_words, lineCC->Dr.a, lineCC->Dr.m1, lineCC->Dr.mL, lineCC->Dr.l1_words,
                         lineCC->Dr.llc_words, lineCC->Dw.a, lineCC->Dw.m1, lineCC->Dw.mL, lineCC->Dw.l1_words,
                         lineCC->Dw.llc_words, lineCC->Bc.b, lineCC->Bc.mp, lineCC->Bi.b, lineCC->Bi.mp);
        } else if (clo_cache_sim && !clo_branch_sim) {
            VG_(fprintf)(fp,
                         "%d %llu %llu %llu %llu %llu"
                         " %llu %llu %llu %llu %llu"
            ...
            VG_(fprintf)(fp,
                         "%d %llu"
                         " %llu %llu %llu %llu\n",
                         lineCC->loc.line, lineCC->Ir.a, lineCC->Bc.b, lineCC->Bc.mp, lineCC->Bi.b, lineCC->Bi.mp);
        } else {
    ...
    if (clo_cache_sim && clo_branch_sim) {
        VG_(fprintf)(fp,
                     ...
                     " %llu %llu %llu %llu %llu"
                     " %llu %llu %llu %llu\n",
                     Ir_total.a, Ir_total.m1, Ir_total.mL, Ir_total.l1_words, Ir_total.llc_words, Dr_total.a,
                     Dr_total.m1, Dr_total.mL, Dr_total.l1_words, Dr_total.llc_words, Dw_total.a, Dw_total.m1,
                     Dw_total.mL, Dw_total.l1_words, Dw_total.llc_words, Bc_total.b, Bc_total.mp, Bi_total.b,
                     Bi_total.mp);
    } else if (clo_cache_sim && !clo_branch_sim) {
        VG_(fprintf)(fp,
                     "summary:"
                     ...
                     Ir_total.a, Ir_total.m1, Ir_total.mL, Ir_total.l1_words, Ir_total.llc_words, Dr_total.a,
                     Dr_total.m1, Dr_total.mL, Dr_total.l1_words, Dr_total.llc_words, Dw_total.a, Dw_total.m1,
                     Dw_total.mL, Dw_total.l1_words, Dw_total.llc_words);
    } else if (!clo_cache_sim && clo_branch_sim) {
        VG_(fprintf)(fp,
                     "summary:"
                     " %llu"
...
#define CG_MAX(a, b) ((a) >= (b) ? (a) : (b))
    ...
    * width. */
    ...
    if (clo_cache_sim) {
        ...
        VG_(umsg)("I1 avg usage:  %*.2f%%\n", l1, Ir_total.l1_words * 4 * 100.0 / I1.size);
        VG_(umsg)("\n");
       ...
       * determine the width of columns 2 & 3. */
        ...
        if (0 == Dw_total.a)
            Dw_total.a = 1;
        VG_(umsg)("D1  miss rate: %*.1f%% (%*.1f%%     + %*.1f%%  )\n", l1, D_total.m1 * 100.0 / D_total.a, l2,
                  Dr_total.m1 * 100.0 / Dr_total.a, l3, Dw_total.m1 * 100.0 / Dw_total.a);
        VG_(umsg)("LLd miss rate: %*.1f%% (%*.1f%%     + %*.1f%%  )\n", l1, D_total.mL * 100.0 / D_total.a, l2,
                  Dr_total.mL * 100.0 / Dr_total.a, l3, Dw_total.mL * 100.0 / Dw_total.a);
        VG_(umsg)("D1 avg usage:  %*.2f%%\n", l1, D_total.l1_words * 4 * 100.0 / D1.size);
        VG_(umsg)("\n");
        ...
        LL_a = Dr_total.a + Dw_total.a + Ir_total.a;
        LL_avg_words = (Dr_total.llc_words + Dw_total.llc_words + Ir_total.llc_words * 1.0) / LL_a;
        ...
        VG_(umsg)("LL avg words:  %*.1f  (D %llu  +  I %llu   )\n", l1, LL_avg_words,
                  (Dr_total.llc_words + Dw_total.llc_words), Ir_total.llc_words);
        VG_(umsg)(fmt, "LL misses:    ", LL_total_m, LL_total_mr, LL_total_mw);

        VG_(umsg)("LL miss rate:  %*.1f%% (%*.1f%%     + %*.1f%%  )\n", l1,
                  LL_total_m * 100.0 / (Ir_total.a + D_total.a), l2, LL_total_mr * 100.0 / (Ir_total.a + Dr_total.a),
                  l3, LL_total_mw * 100.0 / Dw_total.a);
        VG_(umsg)("LL local miss: %*.1f%% (%*.1f%%     + %*.1f%%  )\n", l1, LL_total_m * 100.0 / LL_total, l2,
                  LL_total_mr * 100.0 / LL_total_r, l3, LL_total_mw * 100.0 / LL_total_w);
        VG_(umsg)("LL avg usage:  %*.2f%%\n", l1, LL_avg_words * 4 * 100.0 / LL.size);
    }
    ...
    if (clo_branch_sim) {
        ...
        VG_(umsg)("Mispred rate:  %*.1f%% (%*.1f%%     + %*.1f%%   )\n", l1, B_total.mp * 100.0 / B_total.b, l2,
                  Bc_total.mp * 100.0 / Bc_total.b, l3, Bi_total.mp * 100.0 / Bi_total.b);
    }
...
{
    if (VG_(str_clo_cache_opt)(arg, &clo_I1_cache, &clo_D1_cache, &clo_LL_cache)) {
    }

    else if VG_STR_CLO (arg, "--cachegrind-out-file", clo_cachegrind_out_file) {
    } else if VG_STR_CLO (arg, "--cachegrind-mem-file", clo_cachegrind_mem_file) {
    } else if VG_BOOL_CLO (arg, "--cache-sim", clo_cache_sim) {
    } else if VG_BOOL_CLO (arg, "--branch-sim", clo_branch_sim) {
    } else if VG_BOOL_CLO (arg, "--mem-log", clo_mem_log) {
    } else
        return False;
...
static void cg_pre_clo_init(void)
{
    ...
    VG_(details_copyright_author)("Copyright (C) 2002-2017, and GNU GPL'd, by Nicholas Nethercote et al.");
    VG_(details_bug_reports_to)(VG_BUGS_TO);
    VG_(details_avg_translation_sizeB)(500);

    VG_(clo_vex_control).iropt_register_updates_default = VG_(clo_px_file_backed) =
    ...
    VG_(needs_command_line_options)(cg_process_cmd_line_option, cg_print_usage, cg_print_debug_usage);
}
...
static void cg_post_clo_init(void)
{
    ...
    CC_table = VG_(OSetGen_Create)(offsetof(LineCC, loc), cmp_CodeLoc_LineCC, VG_(malloc), "cg.main.cpci.1", VG_(free));
    instrInfoTable = VG_(OSetGen_Create)(/*keyOff*/ 0, NULL, VG_(malloc), "cg.main.cpci.2", VG_(free));
    stringTable = VG_(OSetGen_Create)(/*keyOff*/ 0, stringCmp, VG_(malloc), "cg.main.cpci.3", VG_(free));

    VG_(post_clo_init_configure_caches)(&I1c, &D1c, &LLc, &clo_I1_cache, &clo_D1_cache, &clo_LL_cache);
    ...
    Int largest_load_or_store_size = VG_(machine_get_size_of_largest_guest_register)();
