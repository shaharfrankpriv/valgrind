...
void test_set_used()
{
    ...
    for (int i = 0; tests[i].b; i++) {
        ...
        d = mark_used_bits(t->a, t->s, t->l, &w);
...
void dump_cache_t2(cache_t2* c)
{
    ...
    printf("desc %s assoc %d line_size %d line_size_bits %d sets %d size %d tag_shift %d\n", c->desc_line, c->assoc,
           c->line_size, c->line_size_bits, c->sets, c->size, c->tag_shift);
...
void test_caches()
{
    struct Test {
        ...
        Int llcw;  // expected used words m1
            ...
            {"line 2 (l1 miss, LL hit), word 0 (replace LRU)", .a = 64 * 2, .s = 4, .m1 = 1, .mL = 0, .l1w = 2,
             .llcw = 5},
            {}
    };
    ...
    struct Test igtests[] = {
            {"line 1+2, words first + last (i1 miss, ll hit)", .a = 64 + 60, .s = 8, .m1 = 0, .mL = 0, .l1w = 3,
             .llcw = 7},
    ...
    for (struct Test* t = itests; t->s; t++) {
        ...
        if (icc.m1 != t->m1 || icc.mL != t->mL || icc.l1_words != t->l1w || icc.llc_words != t->llcw) {
            Panic("I CacheLine stats != m1 %d, mL %d, l1w %d, llcw %d", t->m1, t->mL, t->l1w, t->llcw);
        }
    }
    ...
    for (struct Test* t = dtests; t->s; t++) {
        ...
        dump_cache_t2(&LL);
        dump_CacheCC(&dcc);
        if (dcc.m1 != t->m1 || dcc.mL != t->mL || dcc.l1_words != t->l1w || dcc.llc_words != t->llcw) {
            Panic("D CacheLine stats != m1 %d, mL %d, l1w %d, llcw %d", t->m1, t->mL, t->l1w, t->llcw);
        }
    }
    ...
    for (struct Test* t = igtests; t->s; t++) {
        ...
        if (icc.m1 != t->m1 || icc.mL != t->mL || icc.l1_words != t->l1w || icc.llc_words != t->llcw) {
            Panic("I (gen) CacheLine stats != m1 %d, mL %d, l1w %d, llcw %d", t->m1, t->mL, t->l1w, t->llcw);
        }
    }
...
void test_array()
{
    ...
    dump_CacheCC(&icc);
    printf("array counts: a %llu wl1 %llu wllc %llu l1u %%%.2f llcu %%%.2f\n", icc.a, icc.l1_words, icc.llc_words,
           icc.l1_words * 400.0 / icc.a / I1.size, icc.llc_words * 400.0 / icc.a / LL.size);
}
